...
% - OSA Visível: Polinômio grau 2 para TODOS (todos os canais, todas as cores)
%
...
for tipo_idx = 1:length(canal_tipos)
    ...
    % OSA Visível sempre usa polinômio grau 2 para TODOS
    ...
    % Ajuste para VERDE (polinômio grau 2)
    if sum(valid_green) >= 3
        [p_osa_green, S_osa_green] = polyfit(duty_cycles(valid_green), osa_green_mean(valid_green), 2);
        ...
        p_osa_green = [NaN, NaN, NaN];
        y_fit_osa_green = [];
        R2_osa_green = NaN;
        grau_green = 2;
    end
    
    % Ajuste para VERMELHO (polinômio grau 2)
    if sum(valid_red) >= 3
        ...
        p_osa_red = [NaN, NaN, NaN];
        y_fit_osa_red = [];
        R2_osa_red = NaN;
        grau_red = 2;
    end
    
    % Ajuste para AZUL (polinômio grau 2)
    if sum(valid_blue) >= 3
        [p_osa_blue, S_osa_blue] = polyfit(duty_cycles(valid_blue), osa_blue_mean(valid_blue), 2);
    ...
    % Exibir parâmetros
    fprintf('\n  %s - Parâmetros do ajuste (Grau 2):\n', tipo_nome);
    if ~isnan(p_osa_green(1))
        fprintf('    Verde:    y = %.2fx² + %.2fx + %.2f  (R² = %.4f)\n', ...
            p_osa_green(1), p_osa_green(2), p_osa_green(3), R2_osa_green);
    else
        fprintf('    Verde:    Dados insuficientes\n');
    end
    
    if ~isnan(p_osa_red(1))
        fprintf('    Vermelho: y = %.2fx² + %.2fx + %.2f  (R² = %.4f)\n', ...
    ...
    if ~isnan(p_osa_blue(1))
        fprintf('    Azul:     y = %.2fx² + %.2fx + %.2f  (R² = %.4f)\n', ...
            p_osa_blue(1), p_osa_blue(2), p_osa_blue(3), R2_osa_blue);
    else
...
function plotar_osa(duty_cycles, green_mean, green_std, red_mean, red_std, ...
    ...
    if sum(valid_green) >= 3
        ...
        text(1.5, max(green_mean(valid_green))*0.9, ...
            ...
            p_green(1), p_green(2), p_green(3), R2_green), ...
            'FontSize', 9, 'BackgroundColor', 'white', 'EdgeColor', 'black');
    end
    xlabel('Duty Cycle (%)', 'FontSize', 11, 'FontWeight', 'bold');
    ylabel('Intensidade', 'FontSize', 11, 'FontWeight', 'bold');
    title('LED Verde (Grau 2)', 'FontSize', 12, 'FontWeight', 'bold');
    grid on;
    xlim([0, 11]);
    set(gca, 'FontSize', 10);
    hold off;
    ...
    if sum(valid_blue) >= 3
        ...
        text(1.5, max(blue_mean(valid_blue))*0.9, ...
            sprintf('y = %.2fx² + %.2fx + %.2f\nR² = %.4f', ...
            p_blue(1), p_blue(2), p_blue(3), R2_blue), ...
            'FontSize', 9, 'BackgroundColor', 'white', 'EdgeColor', 'black');
    end
    xlabel('Duty Cycle (%)', 'FontSize', 11, 'FontWeight', 'bold');
    ylabel('Intensidade', 'FontSize', 11, 'FontWeight', 'bold');
    title('LED Azul (Grau 2)', 'FontSize', 12, 'FontWeight', 'bold');
    grid on;
    xlim([0, 11]);
    set(gca, 'FontSize', 10);
    hold off;
    ...
    sgtitle(sprintf('OSA Visível - %s (Média de 5 Tomadas - Ajuste Grau 2)', tipo_nome), ...
        'FontSize', 14, 'FontWeight', 'bold');
    
    output_file = sprintf('OSA_Visivel_%s_ajuste_grau2.png', tipo_nome_safe);
    print(output_file, '-dpng', '-r300');

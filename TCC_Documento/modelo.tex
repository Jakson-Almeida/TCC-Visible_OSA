\documentclass[oneside, english]{abntbibufjf}
...
\autorVirg{Almeida, Jakson}
\local{Juiz de Fora}
\data{2025}
\orientador[Orientador:]{Alexandre Bessa dos Santos}
\coorientador[Coorientador:]{Felipe Barino}
\orientadorTitulo{Professor}
\coorientadorTitulo{Doutor}
...
\subsection{\textbf{Seleção e especificação dos componentes}}
...
\subsection{\textbf{Arquitetura óptica e princípios de funcionamento}}
...
\subsection{\textbf{Fabricação da estrutura por impressão 3D}}
...
\subsection{\textbf{Montagem e alinhamento óptico}}
...
\subsection{\textbf{Caracterização do sistema de detecção}}
...
\subsection{\textbf{Arquitetura e plataforma de desenvolvimento}}
...
\subsection{\textbf{Interface gráfica do usuário}}
...
\subsection{\textbf{Etapa 1: calibração preliminar com luz branca}}
...
\subsection{\textbf{Etapa 2: calibração absoluta com lasers de referência}}
...
\subsection{\textbf{Validação da calibração e métricas de precisão}}
...
\subsection{\textbf{Módulo de calibração}}
...
\subsection{\textbf{Fonte de luz e configuração do PWM}}
...
\subsection{\textbf{Setup experimental}}
...
\subsection{\textbf{Extração de picos e registro de dados}}
...
\subsection{\textbf{Análise de dados em MATLAB}}
...
\subsection{\textbf{Modelagem da combinação de canais}}
...
\subsection{\textbf{Resposta intensidade--duty cycle}}
...
\subsection{\textbf{Modelagem por combinação de canais}}
...
Este trabalho \textbf{prova} que é possível obter precisão metrológica compatível com aplicações educacionais e de pesquisa básica em espectroscopia visível utilizando hardware de baixo custo (impressão 3D, webcam, grade de difração) e calibração automatizada via visão computacional. Os experimentos validam empiricamente o OSA Visível, demonstrando que o processo híbrido de calibração (luz branca + lasers de referência) permite atingir incertezas expandidas inferiores a $\pm$0,3 nm e que o modelo de combinação de canais reproduz a intensidade do espectrômetro ThorLabs com $R^2 = 0{,}9999$ na faixa calibrada. Os objetivos propostos no Capítulo 1 foram atendidos.

\textbf{Onde o OSA Visível pode ser usado:} ensino de espectroscopia em instituições com orçamento limitado; caracterização preliminar de fontes de luz em laboratórios de óptica; projetos de pesquisa que não exijam precisão sub-nanômetro; ambientes de monitoramento onde o custo e a portabilidade são prioritários.

\textbf{O que este trabalho entrega que antes não existia (ou existia de forma muito mais cara):} um conjunto hardware--software integrado (espectrômetro impresso em 3D + interface OSA Visível) que realiza análise espectral quantitativa na faixa 380--750 nm com calibração automatizada, acessível por menos de \$200 USD --- em contraste com os mais de \$30.000 dos OSAs comerciais. A inovação reside na migração da garantia de precisão da engenharia óptica de alto custo para algoritmos de visão computacional, mantendo resultados satisfatórios para a faixa visível.
...
As limitações observadas incluem a taxa de detecção de 92\% para o pico vermelho e a diferença de 9 nm no azul. Três fatores adicionais merecem destaque. \textbf{(i) Linearização:} a relação $\lambda(x)$ é modelada por regressão linear, justificada pela aproximação de pequenos ângulos na equação de Fraunhofer (Seção 2.2). Em primeira ordem, o OSA Visível aproxima a distribuição real de comprimento de onda, mas não a representa com perfeição, especialmente nos extremos do espectro (abaixo de 400 nm e acima de 700 nm), onde a não linearidade da grade se manifesta. \textbf{(ii) Recalibração periódica:} a calibração com lasers foi realizada há mais de seis meses; tratando-se de um espectrômetro impresso em 3D, cujas dimensões físicas (posicionamento da grade, fenda, sensor) são críticas, é difícil garantir que o equipamento se comporte de forma idêntica ao longo do tempo sem recalibrar. A recomendação é recalibrar periodicamente ou antes de medições de maior exigência. \textbf{(iii) Método de extração de picos:} no Experimento 1 não se utilizou a mesma abordagem do Experimento 3 (ajuste Lorentziano via Peaks Viewer); a detecção foi feita diretamente com \texttt{scipy.signal.find\_peaks}. A adoção de ajuste de curva nos experimentos de comprimento de onda poderia refinar ainda mais as estimativas de $\lambda$.

\section{TRABALHOS FUTUROS}

\textbf{Modelagem espectral avançada.} Trabalhos futuros devem explorar modelos não lineares (polinômios de ordem superior) para aprimorar a precisão nas extremidades do espectro, bem como técnicas de correção espectral mais sofisticadas para compensar a resposta não uniforme do sensor. Uma linha promissora é a \textbf{modelagem espectral contínua}: estender a abordagem de combinação de canais ($P_{\text{ThorLabs}}(\lambda) = \beta_1(\lambda) P_R + \beta_2(\lambda) P_G + \beta_3(\lambda) P_B$) a coeficientes $\beta_i$ dependentes do comprimento de onda, permitindo mapear a relação OSA Visível--ThorLabs para espectros arbitrários, não apenas picos pontuais. Explorar interpolação de $\beta_i(\lambda)$ ao longo da faixa visível, modelagem da resposta espectral do sensor (matriz Bayer, crosstalk entre canais) e incorporação explícita de saturação nos modelos matemáticos são direções que podem melhorar a acurácia da intensidade em todo o espectro.

\textbf{MVP de SaaS para o OSA Visível.} Outra linha de evolução é a expansão da interface OSA Visível para uma aplicação \textit{web} (Software as a Service). O MVP de um SaaS permitiria que o espectrômetro deixe de ser exclusivamente uma aplicação local: o usuário poderia acessar o dispositivo remotamente, inclusive pelo celular, e realizar aquisição e análise de espectros em tempo real. Isso é ideal para aplicações de monitoramento remoto, laboratórios distribuídos e equipes que precisam compartilhar dados de forma colaborativa. As Figuras \ref{fig:saas_home} e \ref{fig:saas_dashboard} ilustram conceitos de interface web em desenvolvimento, com armazenamento em nuvem, suporte a múltiplos dispositivos e visualização de espectros ao vivo.
...
\bibitem{laserline_modulos} LASERLINE. \textbf{Produtos Linha Industrial - Módulos Laser}. Disponível em: https://www.laserline.com.br/produtos/industrial/modulos-laser. 

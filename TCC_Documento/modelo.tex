...
\finalcatalog{1. Analisador de espectro óptico. 2. Calibração automatizada. 3. Visão computacional. 4. Espectro visível. I. Santos, Alexandre Bessa dos, orient. II. \inseretitulo.}
...
% Título em negrito, subtítulo em peso normal (conforme template UFJF)
\makeatletter
\renewcommand{\inserecapa}{
  \begin{capa}
   \begin{center}
    ...
    \insereautor}
    \vfill
    {\bfseries \inseretitulo \mdseries \IfNonempty{\inseresubtitulo}{: \inseresubtitulo}}
    \vfill
...
Historicamente, OSAs utilizavam arquiteturas baseadas em monocromadores sintonizáveis (varredura), onde um filtro óptico ajustável resolvia os componentes espectrais individualmente. Em contraste, o OSA Visível utiliza uma arquitetura baseada em \textit{array} de detectores (grade/CMOS), que captura todo o espectro simultaneamente, melhorando a velocidade de aquisição \cite{keysight_osa}. A estrutura emprega uma grade de difração como elemento dispersor e um sensor CMOS (típico de webcams) como detector, combinando simplicidade e baixo custo. A especificação detalhada dos componentes e a descrição da construção encontram-se no Capítulo 3.
...
\subsection{Resolução espectral e resolução amostral}

É fundamental distinguir resolução espectral de resolução amostral em OSAs \cite{yokogawa_osa_specs}. A \textbf{resolução espectral} (\textit{wavelength resolution}) refere-se à capacidade física do instrumento de separar componentes espectrais individuais. Ela é determinada pela largura e formato do filtro óptico (monocromador ou, em espectrômetros de grade, pela fenda e pela dispersão): uma fenda estreita permite distinguir detalhes espectrais finos, enquanto uma fenda larga favorece a medição precisa da potência total de picos largos. A \textbf{resolução amostral} (\textit{min. sampling resolution}) refere-se à densidade de pontos ao longo do eixo de comprimento de onda --- o menor passo (\textit{step size}) entre amostras. Um passo de amostragem grosseiro pode provocar perda de detalhes entre pontos, mesmo quando a óptica seria capaz de resolvê-los \cite{yokogawa_osa_specs}. Em OSAs de varredura, há tipicamente uma relação entre ambas (e.g., várias amostras por unidade de resolução espectral); em espectrômetros baseados em \textit{array} de detectores, como o OSA Visível, a resolução amostral é fixada pelo número de pixels e pela dispersão (nm/pixel).
...
A calibração de OSAs para a faixa visível tem sido amplamente explorada na literatura (Seção 2.2 e 2.3). No contexto de espectrômetros de baixo custo, \cite{gaudi_spectrometer} desenvolveram o projeto \textit{Open Fiber Spectrometer}, espectrômetro de código aberto que serviu de base para este trabalho. A metodologia adotada diferencia-se por combinar calibração preliminar com luz branca e calibração absoluta com lasers (532 nm e 650 nm), alcançando precisão adequada a custo inferior a 200 dólares (USD).
...
\begin{enumerate}
    ...
    \item Dispersão Espectral: A luz colimada incide sobre uma grade de difração de transmissão com densidade de 1000 linhas/mm. A relação entre o comprimento de onda ($\lambda$) e o ângulo de difração ($\theta$) é dada pela Equação \ref{eq:diffraction} (Seção 2.1), onde $n$ representa a ordem de difração (geralmente $n=1$), $d = 1\,\mu\text{m}$ é o espaçamento da grade (inverso da densidade de linhas), e $\alpha$ é o ângulo de incidência da luz na grade \cite{fraunhofer_wiki}.
...
O sistema de detecção baseado em webcam USB apresenta características específicas que influenciam o desempenho do espectrômetro. O sensor CMOS utilizado possui resolução de $640 \times 480$ pixels, com taxa de amostragem de 30 fps. A discretização do sinal óptico segue a Equação \ref{eq:sampling}, onde $I(x,y)$ é a intensidade digital no pixel $(x,y)$, $k$ representa os níveis de intensidade (0 a 255 em escala de 8 bits), e $P(k|x,y)$ é a probabilidade do pixel $(x,y)$ registrar a intensidade $k$.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{../SBAI_SBSE_2025___OSA_Visível___Review/data/janela_principal_recursos.png}
...
A aplicação do limiar resulta em uma máscara binária $M(x,y)$ conforme a Equação \ref{eq:threshold}. A utilização de um limiar adaptativo torna o processo robusto a variações na iluminação da fonte de luz branca e às condições ambientais, eliminando a necessidade de ajustes manuais.

\subsubsection{\textit{Cálculo do centróide e regressão linear preliminar}}

Cálculo do Centróide: O centróide $(x_c, y_c)$ é calculado conforme a Equação \ref{eq:centroid}. Este cálculo é vital, pois o espectrômetro OSA Visível, devido à sua construção 3D com tolerâncias inerentes, pode apresentar um desalinhamento rotacional do espectro no sensor CMOS. O centróide fornece um ponto de referência central que permite à regressão linear preliminar definir corretamente a orientação espacial da linha espectral na imagem.
...
Para a calibração utilizando lasers, $N=20$ quadros são capturados em um intervalo homogêneo de tempo (aproximadamente 2 segundos). A média temporal é aplicada conforme a Equação \ref{eq:frame_avg}, reduzindo o ruído aleatório inerente a sensores CMOS de baixo custo e melhorando drasticamente a Relação Sinal-Ruído (SNR), permitindo a detecção mais precisa dos picos de intensidade dos lasers. Em sensores CMOS de baixo custo, o ruído aleatório (ruído de disparo, ruído térmico) é significativo, e a média temporal é uma técnica eficaz e computacionalmente simples para mitigá-lo.

\subsubsection{\textit{Detecção de picos de intensidade}}

A posição exata dos picos de intensidade dos lasers ($x_{532}$ e $x_{650}$) é determinada conforme a Equação \ref{eq:peak}, onde $I(x) = \frac{1}{H}\sum_{y=0}^{H-1} \bar{I}(x,y)$ é a intensidade média ao longo da direção vertical (eixo $y$) para cada posição horizontal $x$, e $H$ é a altura da imagem (480 pixels).

\subsubsection{\textit{Cálculo dos coeficientes de calibração final}}

Utilizando as coordenadas absolutas (pico de pixel, comprimento de onda real), o software calcula os coeficientes finais $a$ e $b$ conforme a Equação \ref{eq:coeficientes}, onde $x_{\text{verd}}$ e $x_{\text{verm}}$ são as posições dos picos dos lasers verde e vermelho, respectivamente (a justificativa para o uso de dois pontos fixos encontra-se na Seção 2.3.2). A Figura \ref{fig:calib_curve} ilustra a curva de calibração obtida experimentalmente.
...
Conforme a Seção 2.3.3, a acurácia é avaliada pelo Erro Quadrático Médio (RMS). A validação experimental (Capítulo 4) reporta incertezas expandidas e concordância com espectrômetro de referência.
...
A fonte de luz utilizada consistiu em três LEDs de cores verde, vermelho e azul, permitindo o acionamento individual de cada cor. A intensidade de cada LED foi controlada mediante modulação por largura de pulso (PWM), com a possibilidade de salvar espectros para cada LED separadamente. As fontes foram montadas no laboratório; não há especificação detalhada de tensão e corrente de operação de cada componente. O controle por PWM é em malha aberta: o duty cycle define apenas o tempo em que cada LED permanece ligado, sem retroalimentação do brilho efetivo. Assim, não se dispõe de informação do brilho óptico real do LED antes da medição no OSA --- a intensidade incidente é inferida somente a partir do espectro medido. Isso implica que a relação entre duty cycle e potência óptica pode variar entre LEDs (e.g., eficiência luminosa diferente) e ao longo do tempo (deriva térmica), afetando a interpretação da sensibilidade do OSA em relação ao brilho da fonte.
...
\begin{itemize}
    ...
    \item Resolução espectral e amostral (conforme Seção 2.2.3 e \cite{yokogawa_osa_specs}): A resolução espectral (capacidade de separar componentes espectrais) é limitada pela largura da fenda óptica e pela dispersão da grade. A resolução amostral (passo de amostragem em $\lambda$) corresponde à dispersão do sistema, de aproximadamente 1,5 nm/pixel (coeficiente $a$ da calibração), resultando em cerca de 250 pontos efetivos na faixa 380--750 nm. O sensor CMOS (640 pixels) define o limite superior de amostras; a fração utilizada depende do alinhamento e da projeção do espectro sobre o detector.
    ...
    \item Duty cycle em malha aberta: No Experimento 3, a variação de intensidade é feita via PWM em malha aberta, sem medição do brilho óptico antes da captura pelo OSA. A discussão de sensibilidade OSA \textit{vs.} brilho do LED é limitada por essa ausência de informação \textit{a priori} sobre a potência luminosa incidente.
\end{itemize}
...
As incertezas expandidas foram calculadas como $U = k \cdot \sigma/\sqrt{n}$, com $k=1{,}96$ (nível de confiança de 95\%) e $\sigma$ o desvio padrão amostral. É importante salientar que essa fórmula é convencional e não assume que a distribuição dos sinais seja gaussiana; as distribuições observadas (Figura \ref{fig:histogramas_visible_osa}) podem apresentar formas não gaussianas, como multimodalidade no pico vermelho. Além disso, as 100 amostras correspondem a uma medição contínua no tempo (a cada $\sim$10 s), em regime de estabilidade da fonte e do espectrômetro. As medições não são, \textit{a priori}, independentes: amostras consecutivas podem exibir correlação temporal (e.g., deriva térmica lenta, flutuações da fonte). Nessas condições, o erro padrão da média $\sigma/\sqrt{n}$ pode subestimar a incerteza real, pois assume independência. Os valores reportados ($\pm$0,134 nm a $\pm$0,288 nm) devem ser interpretados como indicadores de repetibilidade sob o protocolo adotado, dentro do objetivo de precisão de $\pm$2 nm.

A Figura \ref{fig:evolucao_visible_osa} mostra a evolução temporal dos comprimentos de onda para os três picos RGB ao longo das 100 amostras. A Figura \ref{fig:histogramas_visible_osa} apresenta a distribuição estatística, com bins de largura igual à resolução amostral (1,5 nm), média ($\mu$), desvio padrão ($\sigma$) e incerteza expandida indicados.
...
\caption{Distribuição dos comprimentos de onda dos 3 picos principais RGB (100 amostras) -- OSA Visível. Bins de 1,5 nm (resolução amostral).}
...
Observa-se que o espectrômetro ThorLabs apresentou desvios padrão ligeiramente maiores que o OSA Visível para os picos verde e vermelho, possivelmente devido à maior sensibilidade e resolução espectral do equipamento comercial, que detecta variações mais sutis no sinal. As incertezas expandidas foram calculadas com a mesma metodologia do Experimento 1; as mesmas considerações sobre regime de estabilidade, medição contínua e possível correlação temporal aplicam-se, pois as 100 amostras foram extraídas de uma série temporal contínua.

A Figura \ref{fig:evolucao_thorlabs} ilustra a evolução temporal dos comprimentos de onda medidos pelo espectrômetro ThorLabs para os três picos RGB (100 amostras selecionadas). A Figura \ref{fig:histogramas_thorlabs} mostra a distribuição estatística, com bins de 1,5 nm (resolução amostral).
...
\caption{Distribuição dos comprimentos de onda dos 3 picos principais RGB (100 amostras) -- espectrômetro ThorLabs CCT11. Bins de 1,5 nm.}
...
\begin{figure}[ht]
\centering
\includegraphics[width=1\textwidth]{../images/saas_home.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=1\textwidth]{../images/saas_dashboard.png}
...
\bibitem{yokogawa_osa_specs} YOKOGAWA. OSA Specifications Explained. Disponível em: https://cdn.tmi.yokogawa.com/files/content/OSA\_specifications\_explained.pdf. Acesso em: 29 jan. 2026.

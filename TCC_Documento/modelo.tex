...
O hardware do espectrômetro "Osinha" se baseia em uma solução de baixo custo, aproveitando a fabricação aditiva (impressão 3D) para construir a estrutura óptica. O uso da impressão 3D reduz drasticamente os custos e permite a personalização rápida do design, superando a necessidade de polimento de precisão laboriosa, comum em óptica tradicional \cite{stanford_3d_optics}.
...
\begin{enumerate}
    \item \textbf{Fontes de Referência Estáveis:} Uso de lasers altamente estabilizados, cujos comprimentos de onda são monitorados simultaneamente por um \textit{wavemeter} de altíssima acurácia \cite{terra_calibration}.
    ...
    \item \textbf{Modelagem Polinomial Avançada:} Empregando lâmpadas de arco (neon, argônio) para gerar múltiplos picos espectrais conhecidos, ajustados por polinômios de segunda ou terceira ordem para mapear a relação comprimento de onda-pixel \cite{henriksen_calibration}.
\end{enumerate}
...
Para isolar o espectro (o sinal de interesse) do fundo escuro e do ruído residual da imagem, é necessária uma etapa de segmentação. O \textbf{Método de Otsu} é um algoritmo automático de limiarização global que opera maximizando a variância entre classes (\textit{foreground} vs. \textit{background}) no histograma de intensidade da imagem \cite{otsu_wiki}.
...
Limiarização Otsu \cite{otsu_wiki} & Segmentação ROI espectral & Isolamento do sinal de interesse & Automatiza o processo, tornando-o robusto a variações de iluminação \\
